...
VERIFIED asserts the file was diffed against reverse-engineering/export/. That assertion decays the
...
Usage: python reverse-engineering/tools/check-audit-freshness.py [path] [--stale-only] [--requeue]
"""
import os
import re
import sys
...
# Anchored to this file, not to the working directory. It used to be a bare relative path,
# which silently scanned nothing the moment the script moved -- reporting '0 files: 0 errors'
# and looking like a pass.
DEFAULT_ROOT = str(Path(__file__).resolve().parent.parent.parent
                   / 'unity' / 'Assets' / 'com.dreadscripts.unlocked')
...
def requeue(path, stamped, delta):
    ...
    replacement = (
        ...
        f'{nl}// since ({delta}); needs re-checking against reverse-engineering/export/ before the claim is restored.'
    )
