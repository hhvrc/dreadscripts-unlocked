...
[DefaultMember("Item")]
internal class ObjectReflector
{
	[Serializable]
	[CompilerGenerated]
	...
	internal static readonly Dictionary<Type, TypeReflectionData> pageThread = new Dictionary<Type, TypeReflectionData>();
	...
	internal ObjectReflector(object key)
	{
		...
		m_RulesThread = new TypeReflectionData
		{
			members = members,
			fields = fields,
	...
	{
		if (!m_RulesThread.fields.TryGetValue(v, out var value))
		{
			if (!m_RulesThread.properties.TryGetValue(v, out var value2))
			{
				if (!m_RulesThread.methods.ContainsKey(v))
				{
					attr = null;
					return false;
				}
	...
	{
		if (m_RulesThread.fields.TryGetValue(first, out var value))
		{
			value.SetValue(m_ResolverThread, cust);
			return true;
		}
		if (!m_RulesThread.properties.TryGetValue(first, out var value2))
		{
			return false;
		}
	...
	{
		if (!m_RulesThread.methods.TryGetValue(last, out var value))
		{
