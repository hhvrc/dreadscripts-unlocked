...
internal sealed class ADOverhaul
{
		...
		private void DrawSettingsGUI()
		{
			using (new GUILayout.VerticalScope(GUI.skin.box))
			{
				...
				if (_Instance)
				{
					...
					ADOSettings.SearchTest().editorAnimatedFoldouts.DrawContent(ADOEditorUtility.PrepareRequest().m_ExpressionTemplate, null);
					EditorGUI.indentLevel--;
				}
			}
			using (new GUILayout.VerticalScope(GUI.skin.box))
			{
				...
				if (listener)
				{
					...
					{
						ADOSettings.SearchTest().onSceneNameLabels.DrawContent(ADOEditorUtility.PrepareRequest().m_StubTemplate, null);
						if ((bool)ADOSettings.SearchTest().onSceneNameLabels)
						{
							ADOSettings.SearchTest().labelColor.DrawContent(GUIContent.none, true);
						}
					}
					ADOSettings.SearchTest().generalColor.DrawContent(ADOEditorUtility.PrepareRequest()._InstanceTemplate, true);
					ADOSettings.SearchTest().activeColor.DrawContent(ADOEditorUtility.PrepareRequest().m_ListenerTemplate, true);
					ADOSettings.SearchTest().inactiveColor.DrawContent(ADOEditorUtility.PrepareRequest().observerTemplate, true);
					ADOSettings.SearchTest().mixedColor.DrawContent(ADOEditorUtility.PrepareRequest().m_ParameterTemplate, true);
					ADOSettings.SearchTest().selectionColor.DrawContent(ADOEditorUtility.PrepareRequest().importerTemplate, true);
					ADOSettings.SearchTest().handleSizeMultiplier.DrawFieldContent(ADOEditorUtility.PrepareRequest()._CandidateTemplate, true, null);
					EditorGUI.indentLevel--;
				}
			}
			using (new GUILayout.VerticalScope(GUI.skin.box))
			{
				...
				using (new GUILayout.HorizontalScope())
				{
					ADOSettings.SearchTest().onSceneToolSelection.DrawContent(new GUIContent("Tool Overlay", "Displays the tool selection overlay on the scene view."), null);
					using (new EditorGUI.DisabledScope(!ADOSettings.SearchTest().onSceneToolSelection))
					{
						ADOSettings.SearchTest().toolSelectionOverlayAlignment.DrawEnumPopup<ADOEditorUtility.PositionFlag>("Position", requiresb: false, null, Array.Empty<GUILayoutOption>());
					}
				}
				using (new GUILayout.HorizontalScope())
				{
					ADOSettings.SearchTest().onSceneEditingOverlay.DrawContent(ADOEditorUtility.PrepareRequest().messageTemplate, null);
					using (new EditorGUI.DisabledScope(!ADOSettings.SearchTest().onSceneEditingOverlay))
					{
						ADOSettings.SearchTest().toolOverlayAlignment.DrawEnumPopup<ADOEditorUtility.PositionFlag>("Position", requiresb: false, null, Array.Empty<GUILayoutOption>());
					}
				}
				ADOSettings.SearchTest().onSceneTooltip.DrawContent(ADOEditorUtility.PrepareRequest().bridgeTemplate, null);
				EditorGUI.indentLevel--;
			}
		}
		...
		internal static void Draw()
		{
			...
			if (!m_Message)
			{
				...
				CountStruct(InstantiateSystem(list.ToArray())).PublishAccount(delegate(JsonObject response)
				{
					...
					_Importer = response.Item("solution");
					_List = response.Item("complete");
	...
	internal readonly struct JsonObject
	{
		...
		[SpecialName]
		internal JsonValue Item(string reference)
		{
		...
		public string ToString(bool overridedef)
		{
			if (overridedef)
			{
	...
	internal readonly struct JsonValue
	{
		...
		internal JsonValue(string def)
		{
			...
			if (def.Length > 1)
			{
				...
				{
					stringValue = ((def.Length != 2) ? def.Substring(1, def.Length - 2) : string.Empty);
				}
			}
			else
			{
	...
	private class ADOSettings
	{
		...
		internal class BoolSetting : SettingBase
		{
			...
			[SpecialName]
			internal void SetValue(bool useres)
			{
				if (_value != useres)
				{
					_value = useres;
			...
			internal void Draw(string spec, GUIStyle token = null, params GUILayoutOption[] options)
			{
				DrawContent(new GUIContent(spec), token, options);
			}

			internal void DrawContent(GUIContent config, GUIStyle ord = null, params GUILayoutOption[] options)
			{
				if (ord != null)
				{
					SetValue(EditorGUILayout.Toggle(config, GetValue(), ord, options));
				}
				else
				{
					SetValue(EditorGUILayout.Toggle(config, GetValue(), options));
				}
			}

			internal void DrawButton(string asset, string second = null, bool filterneeded = false, Color? res2 = null, Color? value3 = null, params GUILayoutOption[] options)
			{
				DrawButtonContent((!string.IsNullOrEmpty(asset)) ? new GUIContent(asset) : GUIContent.none, (!string.IsNullOrEmpty(second)) ? new GUIContent(second) : GUIContent.none, filterneeded, res2, value3, options);
			}

			internal void DrawButtonContent(GUIContent asset, GUIContent col = null, bool forcerole = false, Color? token2 = null, Color? info3 = null, params GUILayoutOption[] options)
			{
				token2 = token2 ?? GUI.backgroundColor;
				info3 = info3 ?? GUI.backgroundColor;
				Color backgroundColor = GUI.backgroundColor;
				GUI.backgroundColor = ((!GetValue()) ? info3.Value : token2.Value);
				SetValue(GUILayout.Toggle(GetValue(), (!GetValue() && col != null) ? col : asset, (!forcerole) ? GUI.skin.button : EditorStyles.toolbarButton, options));
		...
		internal class FloatSetting : SettingBase
		{
			...
			[SpecialName]
			internal void SetValue(float var1)
			{
				if (_value != var1)
				{
					_value = var1;
			...
			internal void DrawField(string res, bool setresult = true, GUIStyle template = null, params GUILayoutOption[] options)
			{
				DrawFieldContent(new GUIContent(res), setresult, template, options);
			}

			internal void DrawFieldWithLabelWidth(string i, float b, bool isserv = true, GUIStyle spec2 = null, params GUILayoutOption[] options)
			{
				EditorGUIUtility.labelWidth = b;
				DrawFieldContent(new GUIContent(i), isserv, spec2, options);
				EditorGUIUtility.labelWidth = 0f;
			}

			internal void DrawFieldContent(GUIContent asset, bool readresult = true, GUIStyle proc = null, params GUILayoutOption[] options)
			{
				using (new GUILayout.HorizontalScope())
				{
					SetValue((proc == null) ? EditorGUILayout.FloatField(asset, GetValue(), options) : EditorGUILayout.FloatField(asset, GetValue(), proc, options));
			...
			internal void DrawFieldWithLabelWidthContent(GUIContent instance, float connection, bool isdic = true, GUIStyle info2 = null, params GUILayoutOption[] options)
			{
				EditorGUIUtility.labelWidth = connection;
				DrawFieldContent(instance, isdic, info2, options);
				EditorGUIUtility.labelWidth = 0f;
			}

			internal void DrawSlider(string first, float cont, float temp, bool iscounter2 = true, params GUILayoutOption[] options)
			{
				DrawSliderContent(new GUIContent(first), cont, temp, iscounter2, options);
			}

			internal void DrawSliderContent(GUIContent reference, float second, float consumer, bool isord2 = true, params GUILayoutOption[] options)
			{
				using (new GUILayout.HorizontalScope())
				{
					SetValue(EditorGUILayout.Slider(reference, GetValue(), second, consumer, options));
			...
			internal void DrawNormalizedSlider(string instance, bool containsselection = true, params GUILayoutOption[] options)
			{
				DrawNormalizedSliderContent(new GUIContent(instance), containsselection, options);
			}

			internal void DrawNormalizedSliderContent(GUIContent param, bool requirescaller = true, params GUILayoutOption[] options)
			{
				DrawSliderContent(param, 0f, 1f, requirescaller, options);
			}
		...
		internal class EnumSetting : FloatSetting
		{
			...
			internal void DrawIntFieldContent(GUIContent value, GUIStyle caller = null, params GUILayoutOption[] options)
			{
				IntValue = ((caller == null) ? EditorGUILayout.IntField(value, IntValue, options) : EditorGUILayout.IntField(value, IntValue, caller, options));
			}

			internal void DrawIntField(string param, GUIStyle caller = null, params GUILayoutOption[] options)
			{
				DrawIntFieldContent(new GUIContent(param), caller, options);
			}

			internal void DrawEnumPopupContent<T>(GUIContent v, bool ispol = false, GUIStyle util = null, params GUILayoutOption[] options) where T : Enum
			{
				if (!ispol)
				{
					IntValue = ((util != null) ? ((int)(object)EditorGUILayout.EnumPopup(v, (T)(object)IntValue, util, options)) : ((int)(object)EditorGUILayout.EnumPopup(v, (T)(object)IntValue, options)));
				}
				else
				{
					IntValue = ((util != null) ? ((int)(object)EditorGUILayout.EnumFlagsField(v, (T)(object)IntValue, util, options)) : ((int)(object)EditorGUILayout.EnumFlagsField(v, (T)(object)IntValue, options)));
				}
			}

			internal void DrawEnumPopup<T>(string key, bool requiresb = false, GUIStyle res = null, params GUILayoutOption[] options) where T : Enum
			{
				DrawEnumPopupContent<T>(new GUIContent(key), requiresb, res, options);
			}

			internal static EnumSetting FromEnum<T>(T v, Action pred = null) where T : Enum
			{
		...
		internal class VectorSetting : SettingBase
		{
			...
			[SpecialName]
			internal void SetValue(Vector3 instance)
			{
				if (cachedValue != instance)
				{
			...
			internal void Initialize(Vector3 reference, Action result)
			{
			...
			{
				Initialize(new Vector3(last, visitor, third), config2);
			}

			internal VectorSetting(float info, float result, Action temp = null)
			{
				Initialize(new Vector3(info, result), temp);
			}

			internal void DrawVector2FieldContent(GUIContent value, params GUILayoutOption[] options)
			{
				using (new GUILayout.HorizontalScope())
				{
			...
			internal void DrawVector2Field(string v, params GUILayoutOption[] options)
			{
				DrawVector2FieldContent(new GUIContent(v), options);
			}

			internal void DrawVector3FieldContent(GUIContent reference, params GUILayoutOption[] options)
			{
				SetValue(EditorGUILayout.Vector3Field(reference, GetValue(), options));
			}

			internal void DrawVector3Field(string config, params GUILayoutOption[] options)
			{
				DrawVector3FieldContent(new GUIContent(config), options);
			}
		...
		internal class StringSetting : SettingBase
		{
			...
			[SpecialName]
			internal void SetValue(string init)
			{
				if (_value != init)
				{
					_value = init;
		...
		internal class ColorSetting : SettingBase
		{
			...
			[SpecialName]
			internal void SetValue(Color config)
			{
				r = config.r;
				g = config.g;
				b = config.b;
			...
			internal void Draw(string reference, bool canselection = true, params GUILayoutOption[] options)
			{
				DrawContent(new GUIContent(reference), canselection, options);
			}

			internal void DrawContent(GUIContent asset, bool rejectmap = true, params GUILayoutOption[] options)
			{
				using (new GUILayout.HorizontalScope())
				{
					SetValue(EditorGUILayout.ColorField(asset, GetValue(), options));
	...
	private sealed class PhysBoneEditor : Editor
	{
		...
		private sealed class _003C_003Ec
		{
			...
			internal void InsertResolver()
			{
				AwakeSystem(propertyConfig, "Show Gizmos", delegate
				{
					...
					{
						ADOSettings.SearchTest().gizmosActive.SetValue(propertyConfig.boolValue);
					}
				...
				using (new GUIColorScope(GUIColorScope.ColoringType.BG, flag, ADOEditorUtility.m_InitializerTemplate, ADOEditorUtility._AuthenticationTemplate))
				{
					...
					{
						ADOSettings.SearchTest().globalGizmo.SetValue(GUILayout.Toggle(flag, text, GUI.skin.button, GUILayout.ExpandWidth(expand: false)));
					}
				}
			}
			...
			internal void ListResolver()
			{
				...
				{
					ADOSettings.SearchTest().gizmoBoneOpacity.SetValue(EditorGUILayout.Slider("Bone Opacity", ADOSettings.SearchTest().gizmoBoneOpacity, 0f, 1f));
					ADOSettings.SearchTest().gizmoLimitOpacity.SetValue(EditorGUILayout.Slider("Limit Opacitiy", ADOSettings.SearchTest().gizmoLimitOpacity, 0f, 1f));
				}
			}
								...
								using (IEnumerator<VRCAvatarDescriptor.CustomAnimLayer> enumerator = m_Mapping.baseAnimationLayers.Concat(m_Mapping.specialAnimationLayers).GetEnumerator())
								{
									while (enumerator.MoveNext())
									{
										...
										for (int i = 0; i < broadcasterTemplate.Length; i++)
										{
											...
											_003C_003Ec__DisplayClass108_2.m_ThreadConfig = m_ContainerConfig.stringValue + _003C_003Ec__DisplayClass108_2.adapterConfig.suffix;
		...
		private sealed class _003C_003Ec__DisplayClass108_2
		{
			...
			{
				m_PrinterConfig.comparatorConfig.LogoutAccount(m_ThreadConfig, adapterConfig.parameterType, 0f);
		...
		private static bool CollectMethod()
		{
			...
			return _MerchantConfig.activeIndex == 6;
		}

		[SpecialName]
		private static bool RestartMethod()
		{
			return _MerchantConfig.activeIndex == 1;
		}

		[SpecialName]
		private static void ViewMethod(bool isreference)
		{
			_MerchantConfig.SetSelected(1, isreference);
		}

		[SpecialName]
		private static bool QueryMethod()
		{
			return _MerchantConfig.activeIndex == 2;
		}

		[SpecialName]
		private static void OrderMethod(bool injecttask)
		{
			_MerchantConfig.SetSelected(2, injecttask);
		}

		[SpecialName]
		private static bool ConcatMethod()
		{
			return _MerchantConfig.activeIndex == 3;
		}

		[SpecialName]
		private static void LogoutMethod(bool islast)
		{
			_MerchantConfig.SetSelected(3, islast);
		}

		[SpecialName]
		private static bool AddMethod()
		{
			return _MerchantConfig.activeIndex == 4;
		}

		[SpecialName]
		private static void PublishMethod(bool isinstance)
		{
			_MerchantConfig.SetSelected(4, isinstance);
		}

		[SpecialName]
		private static bool RevertMethod()
		{
			return _MerchantConfig.activeIndex == 5;
		}

		[SpecialName]
		private static void ReflectResolver(bool validateident)
		{
			_MerchantConfig.SetSelected(5, validateident);
		}
		...
		private void method_0()
		{
			if (_MerchantConfig.activeIndex < 0)
			{
				return;
			}
		...
		private static void DefineProducer(SceneView spec)
		{
			...
			if (AddMethod())
			{
				...
				using (new GUIColorScope(GUIColorScope.ColoringType.FG, flag, ADOSettings.SearchTest().labelColor.GetValue()))
				{
			...
			if (QueryMethod())
			{
				...
				using (new GUIColorScope(GUIColorScope.ColoringType.FG, flag2, ADOSettings.SearchTest().labelColor.GetValue()))
				{
			...
			if (RevertMethod())
			{
				...
				Handles.color = ADOSettings.SearchTest().selectionColor.GetValue();
				using (new GUIColorScope(GUIColorScope.ColoringType.FG, flag3, ADOSettings.SearchTest().labelColor.GetValue()))
				{
			...
			if (ConcatMethod())
			{
				...
				Handles.color = ADOSettings.SearchTest().selectionColor.GetValue();
				using (new GUIColorScope(GUIColorScope.ColoringType.FG, flag4, ADOSettings.SearchTest().labelColor.GetValue()))
				{
			...
			Event current = Event.current;
			if (Tools.current != Tool.View && !current.alt && (bool)ADOSettings.SearchTest().ignoreSceneClicks && _MerchantConfig.activeIndex > 0 && current.type == EventType.MouseDown && current.button == 0)
			{
		...
		private static void PushProducer(SceneView first)
		{
			...
			int num = _MerchantConfig.activeIndex;
			if (num < 0)
			{
				num = 0;
			}
			...
			{
				ADOEditorUtility.PositionFlag enumValue = ADOSettings.SearchTest().toolSelectionOverlayAlignment.GetEnumValue<ADOEditorUtility.PositionFlag>();
				bool flag2;
				using (new ADOEditorUtility.SceneViewPanel(first, 250f, 34f, enumValue, m_IdentifierStruct))
				{
					Rect lastRect;
					using (new GUILayout.HorizontalScope())
					{
						using (new EditorGUI.DisabledScope(_MerchantConfig.activeIndex <= 0))
						{
							...
							{
								ADOSettings.SearchTest().ignoreSceneClicks.Toggle();
							}
						}
						GUILayout.FlexibleSpace();
						...
						EditorGUI.BeginChangeCheck();
						int activeIndex = EditorGUILayout.Popup(GUIContent.none, num, classConfig, GUILayout.Width(x + 20f));
						if (EditorGUI.EndChangeCheck())
						{
							_MerchantConfig.activeIndex = activeIndex;
							if (_MerchantConfig.activeIndex != 0)
							{
								if (_MerchantConfig.activeIndex != 6)
								{
									VerifyProducer(-1);
									if (_MerchantConfig.activeIndex > 1 && _MerchantConfig.activeIndex < 4)
									{
										StartProducer();
									}
									else
				...
				{
					Handles.BeginGUI();
					ADOSettings.SearchTest().toolSelectionOverlayAlignment.IntValue = (int)ADOEditorUtility.ReflectManager(enumValue, ivk);
					Handles.EndGUI();
				}
			}
		...
		{
			if (!ADOSettings.SearchTest().onSceneEditingOverlay || _MerchantConfig.activeIndex <= 0)
			{
				return;
			}
		...
		private static void ResolveProducer(VRCPhysBone[] key, ADOEditorUtility.BoneChainTree attr, ContainerModelDispatcher res)
		{
			...
			CS_0024_003C_003E8__locals28.m_BridgeConfig = attr.physBone;
			...
			Color value = ADOSettings.SearchTest().generalColor.GetValue();
			Color color = Handles.color;
			Handles.color = value;
			AnimationCurve animationCurveValue = CS_0024_003C_003E8__locals28._GlobalConfig.animationCurveValue;
			List<List<ADOEditorUtility.BoneNode>> chains = attr.chains;
			if (CS_0024_003C_003E8__locals28.m_ReaderConfig.visitorConfig != 1)
			{
				...
				{
					List<ADOEditorUtility.BoneNode> list = chains[i];
		...
		private static void VerifyProducer(int info)
		{
			...
			{
				valConfig = -1;
				_MerchantConfig.SetSelected(6, overridesecond: false);
			}
			else
			{
				valConfig = info;
				_MerchantConfig.Select(6);
			}
			SceneView.RepaintAll();
		}
		...
		private static void InitProducer()
		{
			...
			{
				registryConfig = true;
				float[] array = new float[ADOEditorUtility._BroadcasterTemplate.Count((ADOEditorUtility.PhysBoneParameter p) => p.hasBackingField)];
	...
	private sealed class _003C_003Ec
	{
		...
		internal Rect GetRules()
		{
			using (new GUILayout.HorizontalScope())
			{
				...
				using (new GUIColorScope(GUIColorScope.ColoringType.FG, ordclose, ADOEditorUtility.m_InitializerTemplate, ADOEditorUtility.infoTemplate))
				{
					...
					{
						ADOSettings.SearchTest().hideToolsDuringTesting.Toggle();
						Tools.hidden = false;
					}
				}
				GUILayout.FlexibleSpace();
		...
		internal void FindRules(JsonObject response)
		{
			...
			{
				state = false;
				ADOSettings.SearchTest().a_HasSucceededLastVerification.SetValue(useres: true);
		...
		{
			ADOSettings.SearchTest().a_VerifyOnDisplay.Toggle();
			ADOSettings.SearchTest().a_VerifyOnProjectLoad.SetValue(useres: false);
		}

		internal void SelectTokenizer()
		{
			ADOSettings.SearchTest().a_VerifyOnProjectLoad.Toggle();
			ADOSettings.SearchTest().a_VerifyOnDisplay.SetValue(useres: false);
		}
		...
		internal async Task PrepareTokenizer()
		{
			...
			ADOSettings.SearchTest().u_updateHidden.SetValue(useres: true);
			SetStruct();
		}
	}
	...
	private sealed class _003C_003Ec__DisplayClass132_0
	{
		...
		internal void PostTokenizer()
		{
			...
			ADOSettings.SearchTest().a_HasSucceededLastVerification.SetValue(useres: false);
	...
	private sealed class _003C_003Ec__DisplayClass132_3
	{
		...
		internal void CalcTokenizer()
		{
			try
			{
				string text = _CreatorModel.Item("date");
				...
				_Publisher = _CreatorModel.Item("username");
				product = _CreatorModel.Item("variant");
				decorator = _CreatorModel.Item("token");
				FlushSystem();
				RegisterSystem();
				string info = _CreatorModel.Item("message");
				if (!_Exporter)
				{
					StopStruct(info);
				}
	...
	private sealed class _003C_003Ec__DisplayClass179_0
	{
		...
		{
			visitor = candidateModel.Item("transfer_email");
			_Filter = true;
		}
	}
	...
	private static void CallSystem()
	{
		if (producer && _Predicate && !_Server)
		{
			...
			{
			case 2:
				ADOSettings.SearchTest().hasReadColliderTestingWarning.SetValue(useres: true);
				break;
			case 0:
				WriteSystem();
				break;
	...
	private static void CreateSystem(bool isres)
	{
		...
		if ((!ADOSettings.SearchTest().a_VerifyOnDisplay.GetValue() && !ADOSettings.SearchTest().a_VerifyOnProjectLoad.GetValue() && !isres) || (state && !_Helper) || m_Consumer)
		{
			return;
		}
		_Helper = false;
	...
	{
		bool num = asset.Item("success");
		string text = asset.Item("message");
		string text2 = asset.Item("url");
		bool flag = !string.IsNullOrEmpty(text2);
		string text3 = asset.Item("url_name");
		...
		bool flag2 = asset.Item("wait_warn");
		float num2 = asset.Item("wait_time");
		m_Token |= flag2;
		if (!(num2 <= 0f))
		{
		...
		using (new GUILayout.VerticalScope(EditorStyles.helpBox))
		{
			...
			m_Process.InsertManager(delegate
			{
				...
				using (new GUILayout.HorizontalScope())
				{
					...
					{
						ADOSettings.SearchTest().u_announcementHidden.SetValue(useres: true);
						ADOSettings.SearchTest().u_announcementHiddenDate.SetValue(DateTime.UtcNow.ToString(CultureInfo.InvariantCulture));
					}
				}
			}, DeleteStruct);
	...
	{
		if (ConcatTemplate() && !string.IsNullOrWhiteSpace(ADOSettings.SearchTest().u_updateVersion.GetValue()))
		{
	...
	private static void ResolveStruct(bool isident)
	{
		...
		{
			m_Composer = true;
			string value = ADOSettings.SearchTest().u_announcement.GetValue();
			using (new ADOSettings.SettingsDeferScope())
			{
				ADOSettings.SearchTest().u_updateLink.SetValue(response.Item("download_link"));
				ADOSettings.SearchTest().u_updateMessage.SetValue(response.Item("download_message"));
				ADOSettings.SearchTest().u_updateChangelog.SetValue(response.Item("changelog_link"));
				ADOSettings.SearchTest().u_updateVersion.SetValue(response.Item("version"));
				ADOSettings.SearchTest().u_updateDay.SetValue(CollectSystem());
				ADOSettings.SearchTest().u_announcement.SetValue(response.Item("announcement"));
				if (!string.IsNullOrWhiteSpace(ADOSettings.SearchTest().u_announcement))
				{
					ADOSettings.SearchTest().u_announcement.SetValue(ADOSettings.SearchTest().u_announcement.GetValue().Replace("\\\\n", "\n").Replace("\\n", "\n"));
				}
				ADOSettings.SearchTest().u_announcementLink.SetValue(response.Item("announcement_link"));
				ADOSettings.SearchTest().u_announcementLinkName.SetValue(response.Item("announcement_link_name"));
			}
			if (value != ADOSettings.SearchTest().u_announcement.GetValue())
			{
				ADOSettings.SearchTest().u_announcementHidden.SetValue(useres: false);
			}
	...
	private static void ConnectStruct(bool iskey)
	{
		if ((bool)ADOSettings.SearchTest().u_announcementHidden)
		{
			...
			{
				ADOSettings.SearchTest().u_announcementHidden.SetValue((DateTime.UtcNow - result).TotalDays < 7.0);
			}
			else
			{
				ADOSettings.SearchTest().u_announcementHidden.SetValue(useres: false);
		...
		if (!(_Connection < new SemVer(ADOSettings.SearchTest().u_updateVersion.GetValue())))
		{
			if (!iskey)
			{
				ADOSettings.SearchTest().u_updateHidden.SetValue(useres: true);
		...
		if (iskey)
		{
			ADOSettings.SearchTest().u_updateHidden.SetValue(useres: false);
	...
	internal static void ComputeStruct(SceneView def, Func<Rect> token, Action state, float second2, float connection3)
	{
		...
		ADOEditorUtility.PositionFlag enumValue = ADOSettings.SearchTest().toolOverlayAlignment.GetEnumValue<ADOEditorUtility.PositionFlag>();
		bool flag;
		using (new ADOEditorUtility.SceneViewPanel(def, second2, connection3, enumValue, _Serializer))
		{
			Rect rect = token();
		...
		{
			Handles.BeginGUI();
			ADOSettings.SearchTest().toolOverlayAlignment.IntValue = (int)ADOEditorUtility.ReflectManager(enumValue, ivk);
			Handles.EndGUI();
		}
	}
	...
	internal static async void ChangeStruct(ProcessRunner[] param, Action cust, CancellationTokenSource control)
	{
		try
		{
			...
			while (!param.All((ProcessRunner p) => p.isFinished))
			{
