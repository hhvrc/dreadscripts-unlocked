...
//   static FlushQueue       -> HintLabel(Rect, ...),                line 5902
//   static ConnectQueue     -> HintLabel(Rect, ...),                line 5907
//   static CalculateQueue   -> HintLabel(text, ...),                line 5923
//   static TestQueue        -> HintLabel(text, ...),                line 5928
...
namespace DreadScripts.ControllerEditor
{
    internal static partial class EditorUtils
    {
        ...
        /// <summary>
        /// Calls <paramref name="onScroll"/> with the vertical scroll delta when the wheel is used
        /// over <paramref name="rect"/>.
        /// </summary>
        /// <remarks>
        /// Does not consume the event, so the enclosing scroll view still scrolls. A caller that
        /// wants exclusive use of the wheel has to call <c>Event.current.Use()</c> itself.
        /// </remarks>
        ...
        /// <summary>
        /// Draws a short hint over a control -- the greyed placeholder text a field shows when it
        /// is empty, or a unit suffix at the far end of one.
        /// </summary>
        /// <param name="draw">
        /// False to skip entirely, so a caller can pass a condition instead of wrapping the call.
        /// </param>
        /// <param name="leftInset">
        /// Space to reserve at the left. Only used to decide whether the rect is wide enough to be
        /// worth drawing in; it does not move the text.
        /// </param>
        /// <param name="rightInset">
        /// Space to reserve at the far end. This one does move the text, by that much plus 2.5
        /// pixels, in whichever direction <paramref name="alignLeft"/> selects.
        /// </param>
        /// <param name="alignLeft">
        /// Left-align the hint and inset from the left; false right-aligns and insets from the
        /// right.
        /// </param>
        internal static void HintLabel(Rect rect, string text, bool draw = true, float leftInset = 0f,
            float rightInset = 0f, bool alignLeft = true, GUIStyle style = null)
        {
            if (!draw || rect.width <= leftInset + rightInset)
            {
                return;
            }

            rect.x += alignLeft ? rightInset + 2.5f : -(rightInset + 2.5f);
            GUI.Label(rect, text, style ?? (alignLeft ? styles.noteLeft : styles.noteRight));
        }

        /// <summary>
        /// <see cref="HintLabel(Rect, string, bool, float, float, bool, GUIStyle)"/> over the last
        /// layout control.
        /// </summary>
        internal static void HintLabel(string text, bool draw = true, float leftInset = 0f,
            float rightInset = 0f, bool alignLeft = true)
        {
            HintLabel(GUILayoutUtility.GetLastRect(), text, draw, leftInset, rightInset, alignLeft);
        }
