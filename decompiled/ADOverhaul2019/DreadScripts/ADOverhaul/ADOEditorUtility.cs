...
internal static class ADOEditorUtility
{
		...
		public void ConnectRequest(Rect ident, PositionFlag b = PositionFlag.Right | PositionFlag.Left, PositionFlag util = PositionFlag.Middle, float selection2 = 4f)
		{
			...
			ResizeZone[] array = new ResizeZone[8]
			{
				new ResizeZone
				{
					...
					index = 0,
					rect = new Rect(ident.x - selection2, ident.y + selection2, num, ident.height - num)
				},
				new ResizeZone
				{
					position = PositionFlag.TopLeft,
					index = 1,
					rect = new Rect(ident.x - selection2, ident.y - selection2, num, num)
				},
				new ResizeZone
				{
					position = PositionFlag.Top,
					index = 2,
					rect = new Rect(ident.x + selection2, ident.y - selection2, ident.width - num, num)
				},
				new ResizeZone
				{
					position = PositionFlag.TopRight,
					index = 3,
					rect = new Rect(ident.x + ident.width - selection2, ident.y - selection2, num, num)
				},
				new ResizeZone
				{
					position = PositionFlag.Right,
					index = 4,
					rect = new Rect(ident.x + ident.width - selection2, ident.y + selection2, num, ident.height - num)
				},
				new ResizeZone
				{
					position = PositionFlag.BottomRight,
					index = 5,
					rect = new Rect(ident.x + ident.width - selection2, ident.y + ident.height - selection2, num, num)
				},
				new ResizeZone
				{
					position = PositionFlag.Bottom,
					index = 6,
					rect = new Rect(ident.x + selection2, ident.y + ident.height - selection2, ident.width - num, num)
				},
				new ResizeZone
				{
					position = PositionFlag.BottomLeft,
					index = 7,
					rect = new Rect(ident.x - selection2, ident.y + ident.height - selection2, num, num)
				}
			};
			...
			while (true)
			{
				...
				if (num2 >= array2.Length)
				{
					...
					PositionFlag position = array[m_MapperTemplate].position;
					...
					if (position > PositionFlag.Bottom)
					{
						...
						{
							if (position == PositionFlag.TopRight)
							{
						...
						else if (position == PositionFlag.BottomRight)
						{
							m_StatusTemplate += vector.x;
							if (GetRequest())
							{
				...
				else
				{
					...
					if ((resizeZone.position & b) < resizeZone.position)
					{
						goto IL_03cb;
					}
				...
				goto IL_0324;
				IL_03d8:
				CallManager(resizeZone.rect, pred);
				...
				if (flag && current.type == EventType.MouseDown && rect.Contains(current.mousePosition))
				{
					...
					m_MapperTemplate = resizeZone.index;
	...
	internal class SceneViewPanel : IDisposable
	{
		...
		public SceneViewPanel(SceneView init, float cust, int position_third, float map2 = 20f, PositionFlag v3 = PositionFlag.BottomRight, ResizeHandle caller4 = null)
		{
			...
			Rect rect2 = GetAnchoredRect(rect, cust, position_third, map2, v3, widthIsPercentage);
			if (caller4 != null)
			{
				rect2 = caller4.VerifyRequest(rect2, v3, proc);
				caller4.ConnectRequest(rect2, v3.RestartAccount(isreg: true));
			}
			area = ViewAccount(rect2);
			if (m_AlgoTemplate)
			{
				area.y += 46f;
			}
		...
		private static Rect GetAnchoredRect(Rect reference, float result, int template_end, float instance2 = 20f, PositionFlag param3 = PositionFlag.Bottom, bool isident4 = false)
		{
	...
	internal class BoneChainTree
	{
		...
		internal void BuildNodes(Transform def, int pred_Y)
		{
			...
			if (isLeaf = list.Count == 0)
			{
				...
				{
					Vector3 pos = def.TransformPoint(physBone.endpointPosition);
	...
	internal readonly struct PhysBoneParameter
	{
		...
		internal PhysBoneParameter(string ident, AnimatorControllerParameterType cont, string temp)
		{
			suffix = ident;
			parameterType = cont;
			valueField = (string.IsNullOrWhiteSpace(temp) ? null : typeof(VRCPhysBoneBase).GetField(temp, BindingFlags.Instance | BindingFlags.Public));
			hasBackingField = valueField != null;
		}

		internal float GetFloat(VRCPhysBoneBase reference)
		{
			return (float)valueField.GetValue(reference);
		}

		internal bool GetBool(VRCPhysBoneBase res)
		{
			return (bool)valueField.GetValue(res);
		}

		public string GetValueString(VRCPhysBoneBase asset)
		{
			if (parameterType == AnimatorControllerParameterType.Bool)
			{
				return GetBool(asset).ToString();
			}
			return GetFloat(asset).ToString();
		}
		...
		if (!(value == null))
		{
			using (ReadableTexture readableTexture = new ReadableTexture(value))
			{
				...
				for (int i = 0; i < height; i++)
				{
					...
					{
						if (texture.GetPixel(j, i).a >= caller)
						{
							if (j < num)
							{
								num = j;
